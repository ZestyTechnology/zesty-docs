...
.. figure:: images/1_step01.svg
    ...
    * You will use the Worm bearigs (part P) and both the shell housings (parts A and B)
...
.. figure:: images/1_step03.svg
    ...
    * To assemble the gear shaft take the following parts:
      ...
      * Gear shaft bearing (Part Q) 2x
      * Gear shaft/Hobbed wheel (Part J)
      * Gear wheel (H)

    * Notice that on the end of the plastic part of the gear shaft we have included a little indicator to show you that the gear is turning. The shafts are so smooth that you would not be able to see it otherwise. If you want you can use a permanent marker to paint the top surface of the indicator to make it more visible.
...
.. figure:: images/1_step05.svg
    ...
    * Close the Nimble by placing the Top shell half. Make sure the worm gear is seated nicely in the bearing and that the seam is completely closed. If not, it means that the bearings for the Worm are not properly seated. Press down until it is fully closed.
...
.. figure:: images/1_step07.svg
    ...
    When mountng on plate, use Groove mount adaptor
    ...
    * The overall procedure if using the Groove Mount Adapter (Part D) is have the Hot end in place, place the Groove mount adapter over the top of the hot end and place the Nimble on top of that, making sure the little tabs fit inside the "forks" of the Nimble. This locks the Nimble to the hot end and keeps it centered.
...
.. figure:: images/1_step08.svg
    ...
    Using another mount, check instructons. 
...
.. figure:: images/1_step12.svg
    ...
    * Rotate the Coupler until the bottom set screws are perpendicular to the flat on the stepper shaft. Make sure there still is approx 1.5 mm space between the stepper base and the Coupler and tighten the bottom set screw (part Y).
...
.. figure:: images/1_step13.svg
    ...
    * Clamp the sleeve using the M3 screw (part Z) and the Half nut (part W). Tighten the screw nice and tight. There is a little ridge inside the sleeve clamp to prefent you from squeezing the sleeve too far and jamming the drive cable.
...
.. figure:: images/1_step18.svg
    ...
    * Insert the drive cable into the worm and then slide it down. To place the sleeve clamp correctly, make sure you line up the bolt holes of the sleeve clamp and the Nimble and then with a rocking motion, slip the little lip into the space provided in the Nimble. Push down and the sleeve clamp should slip into position. This can take a little push the first time. As long as you make sure it is aligned properly, this is not a problem.
...
.. figure:: images/1_step19.svg
    ...
    * Now use the remaining screw (part T) to bolt the Nimble to the printhead or effector. The Nimble is now installed. Have a good look to see if all looks the way you think it should and correct any things that do not look right. 
...
First run the extruder a little, with no filament clamped. Just to bed the gears and drive cable in. Extrude and retract a few times. (You will have to switch off the temperature control as most controllers will not move the extruder stepper unless the hot end it up to temperature)
...
To insert filament, open the breech block. You do this by squeezing together the "ears" of the breech block and pulling out. You can leave the shafts of the breech block in or, for better visibility, take the whole breech block out. 
